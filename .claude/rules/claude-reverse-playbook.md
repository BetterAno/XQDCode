---
description: Claude Code 逆向操作手册（详细 Phase、调试路线、MCP 分工、命令与 Hook 规范）
alwaysApply: false
---

# Claude Code 逆向操作手册

> 本文件是 `claude-reverse.md` 的配套操作手册。
> 当需要详细步骤、路线选择、MCP 分工、命令示例或 Hook 细节时再读取。
> 若与主规则冲突，始终以 `claude-reverse.md` 为准。

## 1. 术语约定

- **调试路线**：指 Phase 1/2 使用的浏览器与抓包/断点方案，即路线 A/B/C
- **工作流技能路线**：指 `web-reverse-workflow`、`web-reverse-master` 及相关子 skill 的组织方式
...
- 产出：
  ...
  - 易踩坑点
- 落盘：
  - 把摘要、候选方向、已排除方向写入 `sites/{domain}/docs/notes.md`
...
> `roxybrowser-mcp-server` 和 `js-reverse-mcp` 是两个独立的浏览器进程，不能协同共享状态。
...
- 默认入口：`web-reverse-workflow`
- 方案未定：`web-reverse-brainstorming`
- 已定路线、准备拆步骤：`web-reverse-writing-plans`
- 已有计划、准备执行：`web-reverse-executing-plans`
- 正在写复现器：`web-reverse-test-driven-development`
- 结果不一致：`web-reverse-systematic-debugging`
- 强混淆：`ast-deobfuscation`
- 明确是重型一体化场景时，才直接使用 `web-reverse-master`

## 6. MCP 分工

| Phase | 路线 A | 路线 B | 路线 C |
|-------|--------|--------|--------|
| Phase 1 | RoxyBrowser + Hook | `js-reverse-mcp` 抓请求 | RoxyBrowser + Python CDP |
...
| 场景 | 检测 | 处理 |
|------|------|------|
| RoxyBrowser 未启动 | `check-status` 失败 | 提示用户启动客户端 |
...
Claude Code 使用 bash（Git Bash）作为默认 shell：

| 操作 | 命令 |
|------|------|
| 创建目录 | `mkdir -p path/to/dir` |
| 下载文件 | `curl -L -o PATH URL` |
| Python 执行 | `C:\Users\15219\Desktop\ClaudeProject\.venv\Scripts\python.exe script.py` |
| 查看文件前 N 行 | `head -N PATH` |
| 多条命令 | `cmd1 && cmd2` |
