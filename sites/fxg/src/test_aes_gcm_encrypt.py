...
def generate_captcha_body(trajectory, distance, session_id, tip_y=68):
    ...
    print(f"原始数据长度: {len(json_bytes)} 字节")
    print(f"原始数据前100字符: {json_str[:100]}")
    ...
    print(f"加密数据前50字节 (hex): {encrypted_data[:50].hex()}")
    
    # 构建最终格式: 可能需要添加头部信息
    # 根据真实数据分析，头部是 "tc\x07\x10\x00\x00" + 一些信息
    header = b"tc\x07\x10\x00\x00"
    # 添加一些元数据
    meta = f"000000006a008406".encode('ascii')
    
    # 组合: header + meta + IV + encrypted_data
    # 注意: 实际格式可能不同，这里只是模拟
    final_data = header + meta + iv + encrypted_data
    
    print(f"最终数据长度: {len(final_data)} 字节")
    print(f"最终数据前50字节 (hex): {final_data[:50].hex()}")
...
def test_captcha_body():
    """测试生成 captchaBody"""
    print("=== 测试生成 captchaBody ===")
    ...
    print(f"\n生成的 captchaBody:")
    print(captcha_body[:100] + "..." if len(captcha_body) > 100 else captcha_body)
    ...
    print(f"\n真实 captchaBody 长度: {len(real_captcha_body)}")
    print(f"真实 captchaBody 前50字符: {real_captcha_body[:50]}")
    
    # 解码真实数据对比
    try:
        real_decoded = base64.b64decode(real_captcha_body)
        print(f"\n真实数据解码后长度: {len(real_decoded)}")
        print(f"真实数据前50字节 (hex): {real_decoded[:50].hex()}")
        print(f"真实数据头部: {real_decoded[:16].hex()}")
