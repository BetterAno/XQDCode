"""验证新的 sign 公式: MD5(clientIp + checkId + uuid + trackString)"""
import base64, json, hashlib
from Crypto.Cipher import AES
from Crypto.Util.Padding import unpad

key = '4LEJbupFf627EKJ0'
uuid = '2e812f6b-a5c0-4bc6-8d7b-7772e5740483'
check_id = '6DAF601139764C6AA8F9DA6DBF149508'
client_ip = '113.247.41.241'
encrypted_b64 = ''

cipher = AES.new(key.encode('utf-8'), AES.MODE_ECB)
decrypted = unpad(cipher.decrypt(base64.b64decode(encrypted_b64)), AES.block_size)
data = json.loads(decrypted.decode('utf-8'))

track_str = ''.join([f"{t['x']}{t['y']}{t['time']}" for t in data['track']])

actual_sign = data['sign']

# 新公式: clientIp + checkId + uuid + trackString
raw1 = f'{client_ip}{check_id}{uuid}{track_str}'
calc1 = hashlib.md5(raw1.encode()).hexdigest()

# 对比旧公式
raw2 = f'{client_ip}{key}{uuid}{track_str}'
calc2 = hashlib.md5(raw2.encode()).hexdigest()

print(f'实际 sign:       {actual_sign}')
print(f'新公式(ip+checkId+uuid+track): {calc1}  匹配={calc1 == actual_sign}')
print(f'旧公式(ip+key+uuid+track):     {calc2}  匹配={calc2 == actual_sign}')
