"""解密浏览器中的 checkCode 请求体，验证 sign 计算方式"""
import base64, json, hashlib
from Crypto.Cipher import AES
from Crypto.Util.Padding import unpad

# 浏览器捕获的数据
key = '4LEJbupFf627EKJ0'
uuid = '2e812f6b-a5c0-4bc6-8d7b-7772e5740483'
check_id = '6DAF601139764C6AA8F9DA6DBF149508'
client_ip = '113.247.41.241'
encrypted_b64 = ''

try:
    cipher = AES.new(key.encode('utf-8'), AES.MODE_ECB)
    decrypted = unpad(cipher.decrypt(base64.b64decode(encrypted_b64)), AES.block_size)
    data = json.loads(decrypted.decode('utf-8'))
    print('=== 解密成功！===')
    print(f'sign: {data.get("sign")}')
    print(f'track 点数: {len(data.get("track", []))}')
    if data.get('track'):
        print(f'前3个track: {data["track"][:3]}')
        print(f'后3个track: {data["track"][-3:]}')
    
    # 构建track字符串
    track_str = ''.join([f"{t['x']}{t['y']}{t['time']}" for t in data['track']])
    
    # 方式1: MD5(clientIp + key + uuid + trackStr)
    raw1 = f'{client_ip}{key}{uuid}{track_str}'
    sign1 = hashlib.md5(raw1.encode()).hexdigest()
    
    # 方式2: MD5(clientIp + key + checkId + trackStr)
    raw2 = f'{client_ip}{key}{check_id}{track_str}'
    sign2 = hashlib.md5(raw2.encode()).hexdigest()
    
    # 方式3: MD5(clientIp + key + 无 + trackStr)
    raw3 = f'{client_ip}{key}{track_str}'
    sign3 = hashlib.md5(raw3.encode()).hexdigest()
    
    print(f'\nsign 原值: {data["sign"]}')
    print(f'方式1 (UUID):     {sign1}  匹配={sign1 == data["sign"]}')
    print(f'方式2 (checkId):  {sign2}  匹配={sign2 == data["sign"]}')
    print(f'方式3 (无第三参):  {sign3}  匹配={sign3 == data["sign"]}')
    
except Exception as e:
    print(f'解密失败: {e}')
    import traceback
    traceback.print_exc()
