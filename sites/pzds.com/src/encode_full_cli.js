/**
 * encode_full_cli — generates decode__1174 with sign embedded
 * Usage: node encode_full_cli.js '<body>' '<method>'
 */
...
// Env setup (MUST be before WASM init)
function makeAnchor(){
...
globalThis.navigator={userAgent:'Mozilla/5.0...Chrome/146.0.0.0',webdriver:false,platform:'Win32',language:'zh-CN',languages:['zh-CN'],cookieEnabled:true,hardwareConcurrency:8,deviceMemory:8,maxTouchPoints:0,vendor:'Google Inc.',plugins:{length:5},mimeTypes:{length:2}};
...
// Fix missing globals that script accesses via N.r (globalThis)
...
// Header-like globals — KEEP pzversion/deviceId UNDEFINED so encodeURIComponent
// override detects "undefinedpost" and injects signB64 + body
globalThis.pzversioncode='1';globalThis.pzos='windows';globalThis.pzplatform='pc';globalThis.skey='CLIENT';

// Init WASM NOW (after env is ready)
...
function genSign(data_json,method,timestamp,random){
  var d0,d1;
  try{var rp=_w.__wbindgen_add_to_stack_pointer(-16);
  var p0=psw(data_json,_w.__wbindgen_malloc,_w.__wbindgen_realloc);var l0=_vl;
  var p1=psw(method,_w.__wbindgen_malloc,_w.__wbindgen_realloc);var l1=_vl;
  var p2=psw(timestamp,_w.__wbindgen_malloc,_w.__wbindgen_realloc);var l2=_vl;
  var p3=psw(random,_w.__wbindgen_malloc,_w.__wbindgen_realloc);var l3=_vl;
  _w.generate_sign(rp,p0,l0,p1,l1,p2,l2,p3,l3);
  var r0=gi32()[rp/4+0],r1=gi32()[rp/4+1];d0=r0;d1=r1;
  return gsw(r0,r1);}
...
// Load script
let s=fs.readFileSync(path.join(D,'..','src','script_200_raw.js'),'utf8');
...
if(require.main===module){
  var body=process.argv[2],method=(process.argv[3]||'POST').toLowerCase();
  var ts=process.argv[4]||String(Date.now());
  var rnd=process.argv[5]||String(Math.floor(Math.random()*900000+100000));
  var signHex=process.argv[6];  // optional: pre-computed sign
  if(!body){process.stderr.write('Usage: node encode_full_cli.js <body> [method] [ts] [rnd] [sign]\n');process.exit(1);}
  globalThis.__lastBody=body;
  if(signHex){
    globalThis.__signB64=Buffer.from(signHex).toString('base64');
  }else{
    globalThis.__signB64=Buffer.from(genSign(body,method,ts,rnd)).toString('base64');
  }
  var result=globalThis.__a4('https://api.pzds.com/api/web-client/v2/public/goodsPublic/page',body,method,1);
  var m=result.match(/decode__1174=([^&]+)/);
  process.stdout.write(m?m[1]:'ERR:'+result);
}
