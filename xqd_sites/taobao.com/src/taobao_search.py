...
=====================
三种运行模式:
  模式1: python src/taobao_search.py 耳机                             → 指定关键词，输出浏览器提取脚本
  模式2: python src/taobao_search.py --from-json data.json             → 根据JSON文件：截图+转XLSX
  模式3: python src/taobao_search.py --auto 耳机                        → 一键自动采集（打开浏览器→注入JS→截图→XLSX）
...
HEADERS = [
    "序号", "店铺名", "标题", "售价", "地址",
    "销量", "状态", "详情页链接", "商品图片", "itemId", "商品详情页截图",
]
COL_WIDTHS = [6, 28, 60, 12, 12, 14, 10, 50, 50, 16, 30]
...
def step0_auto_collect(keyword: str, max_pages: int = DEFAULT_PAGES,
    ...
    with sync_playwright() as p:
        ...
        try:
            page.wait_for_selector(
                'a[href*="item.taobao.com/item.htm?id="] img',
                timeout=20000,
            )
        except Exception:
...
def step3_to_xlsx(json_path: str, existing_ids: set = None):
    ...
    for row_idx, item in enumerate(items, 2):
        ...
        for col, val in enumerate(row_data, 1):
            ...
            cell.font = data_font
            cell.border = thin_border
            align_h = "center" if col in (1, 4, 5, 6, 7, 10) else "left"
            cell.alignment = Alignment(horizontal=align_h, vertical="center",
                                       wrap_text=(col in (2, 3, 8, 9)))
