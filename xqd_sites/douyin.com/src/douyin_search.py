    ...
    try:
        ...
        try:
            page.wait_for_selector(
                ...
                ".login-modal, .douyin-login, "
                "#secsdk-captcha-drag-wrapper, .captcha-box",
        ...
        # ---- 第 1 层：URL 检查 ----
        is_video_url = "douyin.com/video/" in url_lower
        if not is_video_url:
            reason = "URL 不是视频详情页"
        ...
        () => {
          const SEL = [
            ...
            '.captcha_verify_bar','.captcha-verify-image',
            // 登录弹窗
            '.login-modal','.douyin-login','.login-mask',
          ...
          const search = (doc) => {
            ...
            // 滑块文案、验证码提示
            if (/请完成安全验证|按住滑块|拖动滑块|验证以确保正常访问|请先登录|登录后即可/.test(txt)) return true;
...
def _keyword_from_json(json_path: str) -> str:
    """从 JSON 文件名中提取关键词。
    文件名格式: {keyword}_douyin_{date}.json
    """
...
def step1_generate_js(keyword: str, max_pages: int = DEFAULT_PAGES):
    ...
    print()
    print("  使用步骤:")
    print(f"    1. 浏览器打开 https://www.douyin.com/search/{keyword}")
    print(f"    2. F12 → Console → 粘贴下方脚本 → 回车")
    print(f"    3. 等待自动滚动加载 → 自动下载 {filename}")
    print(f"    4. 将 {filename} 移到项目根目录")
    print(f"    5. python src/douyin_search.py --screenshot {filename}")
    print(f"    6. python src/douyin_search.py --to-xlsx {filename}")
    print()
    print("-" * 56)
...
def step0_auto_collect(keyword: str, auto_full: bool = False,
    ...
    with sync_playwright() as p:
        ...
        try:
            page.wait_for_selector(
                '[class*="search-result-card"] img[src*="douyinpic"]',
                timeout=20000,
            )
        except Exception:
