...
浏览器:
  标准 Chrome + E:/Projects/ChromeDebug 独立缓存目录（不影响用户正在使用的 Chrome）
...
# 截图延时（秒）
PAGE_WAIT = (1, 2)        # 详情页加载后等待 (min, max)
BETWEEN_WAIT = (1, 2)     # 视频间间隔
    ...
    try:
        ...
        try:
            page.wait_for_selector(
                ".bili-mini-mask, .login-panel-popover, "
                "#captcha_container, .captcha-box, "
                ".bili-captcha, .geetest_panel",
        ...
        # ---- 第 1 层：URL 检查 ----
        is_video_url = "bilibili.com/video/" in url_lower
        if not is_video_url:
            reason = "URL 不是视频详情页"
        ...
        () => {
          ...
          const signals = ['点赞','投币','收藏','分享','关注','粉丝','播放','弹幕','评论'];
          let n = 0; for (const s of signals) { if (t.includes(s)) n++; }
          return n >= 3;
        }
        """)
        if not has_video:
            reason = "页面缺少视频特征（无点赞/投币/收藏等）"
            raise Exception(reason)
        ...
        () => {
          const SEL = [
            '.bili-captcha','.geetest_panel','.geetest_holder',
            '#captcha_container','.captcha-box',
            '.bili-mini-mask','.login-panel-popover',
...
def make_filename(keyword: str) -> str:
    """生成输出文件名: {关键词}_bilibili_{日期}.json"""
...
def step1_generate_js(keyword: str, max_pages: int = DEFAULT_PAGES):
    ...
    print()
    print("  使用步骤:")
    print(f"    1. 浏览器打开 https://search.bilibili.com/all?keyword={keyword}")
    print(f"    2. F12 → Console → 粘贴下方脚本 → 回车")
    print(f"    3. 等待自动翻页加载 → 自动下载 {filename}")
    print(f"    4. 将 {filename} 移到项目根目录")
    print(f"    5. python src/bilibili_search.py --screenshot {filename}")
    print(f"    6. python src/bilibili_search.py --to-xlsx {filename}")
    print()
    print("-" * 56)
...
def step2_screenshot(json_path: str, captcha_pause: bool = True):
    ...
    浏览器: 标准 Chrome + E:/Projects/ChromeDebug 配置
...
def step3_to_xlsx(json_path: str, existing_ids: set = None):
    ...
    # 表头样式（B站粉色主题）
    hdr_fill = PatternFill(start_color="FB7299", end_color="FB7299", fill_type="solid")
