...
class FcBoxLogin:
    def __init__(self):
        ...
        self.js = execjs.compile(open("获取验证载荷信息.js", encoding="utf-8").read())
        self.det = ddddocr.DdddOcr(det=False, ocr=False, show_ad=False, beta=True)
        self.js_login = execjs.compile(open("获取登录载荷信息.js", encoding="utf-8").read())

    def get_captcha(self):
        # 直接使用Python的uuid模块生成UUID v4
        ...
        return response['data']['shadeImageUrl'], response['data']['slideImageUrl'], response['data']['clientIp'], \
            response['data']['checkId'], uuid4, response['data']['key'], response['data']['pointY']
    ...
    @staticmethod
    def generate(target_x: int, fixed_y: int, start_time: int = None):
        """
        生成轨迹
    ...
    def check_captcha(self, clientIp, checkId, uuid4, gj_list, key):
        data = self.js.call('get_data', clientIp, checkId, uuid4, gj_list, key)
    ...
    def login(self, token, public_key, username='123123', password='123123'):
        ...
        item = self.js_login.call('get_login_params', password, token, public_key)
        params = {
            "username": username,
    ...
    def main(self):
        for _ in range(1):
            shadeImageUrl, slideImageUrl, clientIp, checkId, uuid4, key, pointY = self.get_captcha()
            try:
                slide_distance = self.parse_captcha(shadeImageUrl, slideImageUrl)
            except:
                continue
            gj_list = self.generate(target_x=int(slide_distance), fixed_y=pointY)
