...
if (ee == null) {
    ...
    var Be = 0
        , Te = function (D) {
        if ((Be = Be || 0) >= 256 || Me >= 256)
            window.removeEventListener ? window.removeEventListener("mousemove", Te, !1) : window.detachEvent && window.detachEvent("onmousemove", Te);
        else
            try {
                var y = D.x + D.y;
                ee[Me++] = 255 & y,
    ...
    window.addEventListener ? window.addEventListener("mousemove", Te, !1) : window.attachEvent && window.attachEvent("onmousemove", Te)
}
...
\r	\xA0\u2028\u2029`;
            for (g = Object.create(null),
                     y = 0; y < 64; ++y)
                g["ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/".charAt(y)] = y;
            for (g["-"] = 62,
                     g._ = 63,
                     y = 0; y < C.length; ++y)
                g[C.charAt(y)] = -1
        ...
        return D.prototype.toString = function (y) {
            ...
            D.prototype.isProbablePrime = function (y) {
                ...
                if (k.isEven())
                    return !1;
                for (C = 1; C < G.length;) {
                    for (var P = G[C], Q = C + 1; Q < G.length && P < Z;)
                        P *= G[Q++];
                ...
                if (typeof C == "number")
                    if (y < 2)
                        ...
                        for (this.fromNumber(y, k),
                             this.testBit(y - 1) || this.bitwiseTo(D.ONE.shiftLeft(y - 1), c, this),
            ...
            D.prototype.millerRabin = function (y) {
                ...
                (y = y + 1 >> 1) > G.length && (y = G.length);
                for (var Q = Y(), de = 0; de < y; ++de) {
                    Q.fromInt(G[Math.floor(Math.random() * G.length)]);
...
L.ZERO = ie(0),
    L.ONE = ie(1);
(L.prototype.am = function (D, y, C, k, P, Q) {
...
`;
            return (C += y.wordwrap(this.getPrivateBaseKeyB64()) + `
`) + "-----END RSA PRIVATE KEY-----"
        }
        ,
...
`;
            return (C += y.wordwrap(this.getPublicBaseKeyB64()) + `
`) + "-----END PUBLIC KEY-----"
        }
        ,
...
const tt = function () {
    ...
    return D.prototype.setKey = function (y) {
        ...
        ,
        D.prototype.encrypt = function (y) {
            return b(this.getKey().encrypt(y))
        }
        ,
        D.prototype.sign = function (y, C, k) {
            return b(this.getKey().sign(y, C, k))
        }
        ,
        D.prototype.verify = function (y, C, k) {
...
function gen_data(raw_data) {
    const s = sm4.encrypt(
        JSON.stringify(raw_data),
        new TextEncoder().encode("B6*40.2_C9#e4$E3"),
        {mode});
    data = {
        params: u$e(s)
    };
